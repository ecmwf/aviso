...
def test_adding_listener(conf, schema):
    ...
    listener = EventListener(eng, request, [{"type": "Log"}], schema)
...
def test_adding_listeners(conf, schema):
    ...
    eng = engine_factory.create_engine()
    request = {"country": "italy", "date": 20210101}
    listener1 = EventListener(eng, request, [{"type": "Log"}], schema)
    listener2 = EventListener(eng, request, [{"type": "Log"}], schema)
