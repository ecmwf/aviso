...
This API has the same level of expressiveness than the CLI.
...
This component internally relies on Aviso to submit notification to the store.
...
The object ``NotificationManager`` can take as parameter a ``user_config.UserConfig`` object. If not passed the 
manager object will instantiate a config object following the criteria explained in :ref:`configuration`.
Alternatively the user can create and customise a ``UserConfig``. This can be useful to customise the application setup.
