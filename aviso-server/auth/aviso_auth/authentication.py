...
class Authenticator:
    UNAUTHORISED_RESPONSE_HEADER = {
        "WWW-Authenticate": "Bearer realm='auth-o-tron',info='Provide a valid token'"
    }
    ...
    def __init__(self, config, cache=None):
        ...
        if config.authentication_server.get("monitor"):
            self.timer = TimeCollector(
                config.monitoring,
                tlm_type=AvisoAuthMetricType.auth_resp_time.name,
                tlm_name="att"
            )
        ...
        if self.cache:
            logger.debug("Using memoized token validator with cache timeout = %s",
    ...
    def _validate_token_uncached(self, token, x_auth_type, client_ip="unknown"):
        ...
        logger.debug(
            "Calling auth-o-tron /authenticate at %s [auth_type=%s, client_ip=%s]",
            auth_url, x_auth_type, client_ip
        )
        ...
        while n_tries < MAX_N_TRIES:
            ...
            try:
                ...
                logger.debug("validate_token: Token validated successfully [auth_type=%s, ip=%s]", x_auth_type, client_ip)
                return resp
                ...
                if status_code in [401, 403]:
                    logger.warning(
                        ...
                        status_code, x_auth_type, client_ip, resp.text
                    )
                ...
                if status_code == 408 or (500 <= status_code < 600):
                    logger.warning(
                        ...
                        status_code, x_auth_type, client_ip, resp.reason
                    )
                    n_tries += 1
                    time.sleep(random.uniform(1, 5))
                else:
                ...
                logger.warning(
                    ...
                    n_tries + 1, x_auth_type, client_ip, err
                )
                n_tries += 1
                time.sleep(random.uniform(1, 5))
                ...
                logger.error(
                    ...
                    n_tries + 1, x_auth_type, client_ip, e
                )
                raise InternalSystemError("Unexpected error during token validation")

        logger.error("validate_token: Exceeded maximum attempts (%d) [auth_type=%s, ip=%s]",
                     MAX_N_TRIES, x_auth_type, client_ip)
